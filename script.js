...
function triggerEmergency(){
  ...
  // Emergency vehicle always enters from NORTH.
  // It stays centered in the north lane and NEVER sits on top of the AI node.
  emergencyTop = 35;
  emergencyEl.hidden = false;

  emergencyEl.style.left = Math.round(laneCenterX("N") - 23) + "px";
  emergencyEl.style.top = Math.round(emergencyTop) + "px";

  ui.status.innerHTML =
    'Emergency Vehicle: <strong>ACTIVE — APPROACHING FROM NORTH</strong>';
...
function updateEmergency(now,dt){
  ...
  // Stop the ambulance safely before the north stop line.
  // It never enters the central AI/intersection area.
  const targetY = h * 0.35 - 72;
  const speed = 115;
  const move = speed * dt / 1000;

  if(emergencyTop < targetY){
    emergencyTop = Math.min(emergencyTop + move, targetY);
    emergencyEl.style.top = Math.round(emergencyTop) + "px";
  }
